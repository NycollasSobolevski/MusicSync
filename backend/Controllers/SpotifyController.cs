...
public class SpotifyController : ControllerBase
{
    ...
    {
        this.clientUrl = Environment.GetEnvironmentVariable("SPOTIFY_CLIENT_URL");
        this.frontUri = Environment.GetEnvironmentVariable("FRONTEND_URI");
        this.clientId = Environment.GetEnvironmentVariable("SPOTIFY_CLIENT_ID");
        this.clientSecret = Environment.GetEnvironmentVariable("SPOTIFY_CLIENT_SECRET");
        this.redirectCallback = $"{this.frontUri}/Callback?streamer=spotify";
    }
    ...
    )
    {
        try
        {
            ...
            if (token != null)
            {
                ...
                {
                    Data = $"{this.frontUri}/?tab=Spotify"
                });
            }
            ...
            System.Console.WriteLine($"--{this.redirectCallback}--");
            var path = $"https://accounts.spotify.com/authorize?response_type=code&client_id={this.clientId}&scope={scope}&redirect_uri={this.redirectCallback}&state={state}";
            return Ok(new StringReturn
            {
                Data = path
            });
    ...
    )
    {
        ...
        if(debug){
            System.Console.WriteLine($"Redirect: {this.redirectCallback}");
            System.Console.WriteLine($"Authorization: Basic {dataClient}");
        }
        ...
        try
        {
            ...
            if (token.access_token == null || token.refresh_token == null)
                return BadRequest("Error in validate user");
    ...
    )
    {
        ...
        try{
            ...
            if(response.StatusCode != HttpStatusCode.OK){
                return BadRequest("Spotify error!");
            }
            if(debug)
                Console.WriteLine(await response.Content.ReadAsStringAsync());

            SpotifyUserPlaylistItems reqData = await response.Content.ReadFromJsonAsync<SpotifyUserPlaylistItems>(); 
            PlaylistData result = new PlaylistData(){
                ...
                Owner = new PlaylistOwner(){
                    display_name = reqData.owner.display_name,
                    external_urls = reqData.owner.external_urls,
                    id = reqData.owner.id,
                    href = reqData.owner.href
                },
                Public = reqData.@public,
                TracksTotal = reqData.tracks
            };
